...
export default async function DepartmentStatsPage() {
  ...
  // 본부장 권한 확인 (SERVICE_ROLE_KEY 사용 시도 - RLS 우회)
  let profile: { id: string; role: string; department_id: string | null; department_name: string | null } | null = null
  ...
  if (!profile || (profile.role !== 'department_head' && profile.role !== 'branch_head')) {
    console.error('본부장/지사장 권한 없음 또는 프로필 조회 실패:', { profile, profileError })
    redirect('/dashboard')
  }
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-gray-800">
                {profile.department_name || '본부'} 팀원 활동 통계
              </h2>
              <p className="text-sm text-gray-500">
                같은 본부 팀원들의 글 수와 Q&A 수를 확인하세요.
              </p>
            </div>
          </div>
