...
export default async function BlogListPage({
  ...
  let q = supa.from('content_items')
    .select('*')
    .eq('type', 'blog').order('created_at', { ascending: false }).limit(200);
  if (status !== 'all') q = q.eq('status', status);
  const { data: items, error } = await q;
  if (error) console.error('[blog/list]', error.message);

  const { data: counts } = await supa.from('content_items').select('status').eq('type', 'blog');
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="mt-1 text-sm text-slate-500">매일 KST 08:00 자동 생성 · 3,000~5,000자 SEO 톤 · HTML/MD 다운로드</p>
      </div>
      ...
      <div className="mb-6 flex flex-wrap gap-2">
        {STATUSES.map(s => {
          ...
          return (
            <Link key={s.key} href={`/admin/content/blog?status=${s.key}`}
      ...
      {(!items || items.length === 0) ? (
        <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-white p-16 text-center">
          <FileText className="mx-auto w-12 h-12 text-slate-300" />
          <h3 className="mt-4 text-lg font-semibold text-slate-700">생성된 블로그 글이 없습니다</h3>
          <p className="mt-2 text-sm text-slate-500">매일 KST 08:00 자동 생성됩니다.</p>
        </div>
      ) : (
        <div className="rounded-2xl border border-slate-200 bg-white overflow-hidden shadow-sm">
          <ul className="divide-y divide-slate-100">
            {(items ?? []).map((row: { id: string; title: string; status: string; meta_description: string | null; created_at: string; source_refs: { category?: string }[] | null; total_cost_usd: number | null; body_md: string | null }) => {
              const category = row.source_refs?.[0]?.category;
              const score = lintMap.get(row.id);
              const charCount = row.body_md?.replace(/[#*`>_\-]/g, '').length ?? 0;
              return (
                <li key={row.id}>
                  <Link href={`/admin/content/blog/${row.id}`}
                    className="flex items-center gap-4 px-5 py-4 hover:bg-blue-50/40 transition group">
                    ...
                    <ChevronRight className="flex-none w-5 h-5 text-slate-300 group-hover:text-blue-600 group-hover:translate-x-0.5 transition" />
                  </Link>
                </li>
              );
            })}
