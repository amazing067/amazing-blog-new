...
export async function POST(request: NextRequest) {
  try {
    ...
    const { pdfUrl, category } = await request.json()
    ...
    // PDF 파일 다운로드
    console.log('[PDF 파싱] PDF 다운로드 시작:', pdfUrl)
    const pdfResponse = await fetch(pdfUrl)
    
    if (!pdfResponse.ok) {
      return NextResponse.json(
        { error: 'PDF 파일을 다운로드할 수 없습니다' },
        { status: 400 }
      )
    }
    ...
    // 카테고리에 따른 프롬프트 조정
    const categoryHint = category === 'damage' 
      ? '손해보험 상품' 
      : category === 'life' 
      ? '생명보험 상품' 
      : '보험 상품'
    ...
    // 정보 추출 프롬프트
    const extractionPrompt = `다음은 ${categoryHint} 소식지 PDF에서 추출한 텍스트입니다. 이 텍스트에서 다음 정보를 추출해주세요:
