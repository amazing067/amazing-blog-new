/**
 * 대화형 스레드 생성 프롬프트 백업 파일
 * 
 * 원본: lib/prompts/qa-prompt.ts의 generateConversationThreadPrompt 함수
 * 백업 일자: 2025-01-XX
 * 
 * 고급 최적화 전략 적용 전 원본 프롬프트
 * 문제 발생 시 이 파일의 내용으로 롤백 가능
 */

export interface ConversationContextBackup {
  initialQuestion: {
    title: string
    content: string
  }
...
export interface QAPromptDataBackup {
  productName: string
...
/**
 * 원본 generateConversationThreadPrompt 함수
 * 이 함수는 고급 최적화 전략 적용 전의 원본입니다.
 */
export function generateConversationThreadPromptOriginal(
  data: QAPromptDataBackup,
  context: ConversationContextBackup
): string {
