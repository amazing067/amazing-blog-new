...
export function generateInsuranceBlogPrompt(data: PromptData): string {
  let { topic, keywords, product, tone, age, gender, topInsurance, diseaseCodes, designSheetImage, designSheetAnalysis, authorName, searchResults, precedents } = data
...
- **⚠️ 매우 중요 - 담보별 보험료 표시 규칙:**
  ...
  - **월 보험료 총액**은 항상 표시하되, 담보별 보험료는 주제에 맞게 필터링하여 표시

**⚠️ 담보별 보험료 표시 방법 (구체적 가이드):**
...
* **Body (본문):** '기승전결'이 아닌 **'문제제기 -> 심층분석 -> 데이터비교 -> 함정피하기'** 흐름으로 전개하세요.
...
4. **⚠️ 주의사항:**
   ...
   - 검색 결과의 정보를 본문의 맥락에 맞게 자연스럽게 통합
