  ...
  if (isCustomerTurn) {
    ...
    const roleConfig = {
      'customer1': {
        ...
        style: '이전 답변에서 언급된 내용을 참조하되, "하셧는데", "말씀하신" 같은 표현은 가끔만 사용하고(10% 이하), 대부분은 직접 질문으로 시작하세요(90% 이상). 이전 댓글에서 사용한 표현을 절대 반복하지 마세요'
      },
      'customer2': {
        role: '관심자',
...
- **"하셧는데", "말씀하신", "하셨는데" 같은 표현을 연속으로 사용하지 마세요!**
- **이전 댓글에서 사용한 참조 표현("하셧는데", "말씀하신" 등)을 다시 사용하지 마세요**
...
${roleInfo.examples.map(ex => `     * "${ex}"`).join('\n')}
   - **⚠️ 역할에 맞는 댓글을 작성하세요**:
     * ${currentRole === 'customer1' ? '질문자: 이전 답변을 참조하되, "하셧는데", "말씀하신" 같은 표현은 가끔만 사용(10% 이하). 대부분은 직접 질문으로 시작(90% 이상). 이전 댓글에서 사용한 표현 절대 반복 금지' : currentRole === 'customer2' ? '관심자: "저도", "이 부분도" 같은 표현으로 다른 사람임을 명확히 표현, 질문자와는 다른 관점' : currentRole === 'customer3' ? '비교자: "다른 보험사", "비교", "어느 게 나을까요" 같은 표현으로 비교 관점 강조' : '확인자: "정말 그런가요?", "확인하고 싶어요", "혹시" 같은 표현으로 확인 관점 강조'}
   - **⚠️ 다른 역할처럼 행동하지 마세요**: ${currentRole === 'customer1' ? '질문자: "하셧는데", "말씀하신" 같은 표현을 절대 반복하지 마세요. 이전 댓글에서 사용했다면 다른 방식으로 참조하세요(예: 직접 질문, "그럼", "추가로" 등)' : currentRole === 'customer2' ? '관심자가 아닌 다른 사람처럼 행동하지 마세요. "저도", "이 부분도" 같은 표현을 반드시 사용하여 다른 사람임을 명확히 표현하세요' : currentRole === 'customer3' ? '비교자가 아닌 다른 사람처럼 행동하지 마세요. "다른 보험사", "비교" 같은 표현을 반드시 사용하세요' : '확인자가 아닌 다른 사람처럼 행동하지 마세요. "정말", "확인", "혹시" 같은 표현을 반드시 사용하세요'}
...
5. **말투**: 전문 용어를 자연스럽게 사용하되 완전히 이해하지 못한 느낌
...
6. **이전 답변 참조 (매우 중요 - 반복 방지!)**:
   - 첫 답변에서 언급된 내용을 자연스럽게 언급하되, **이전 댓글에서 사용한 참조 표현을 절대 반복하지 마세요**
   - **⚠️ "하셧는데", "말씀하신", "하셨는데" 같은 표현은 가끔만 사용(10% 이하)하고, 대부분은 직접 질문으로 시작하세요(90% 이상)**
   - **다양한 참조 방식** (매번 다르게 선택):
     * 직접 질문 (90% 이상): "[담보명] [조건]에도 적용되나요?" (대부분)
     * 직접 질문 (90% 이상): "[담보명] [금액]으로 말씀하셨는데, [새로운 질문]" (가끔만, 10% 이하)
     * 간접 참조: "그럼 [내용]도 가능한가요?"
     * 간접 참조: "추가로 [내용]도 궁금해요"
     * 간접 참조: "[내용]에 대해 더 알고 싶어요"
   - **⚠️ 절대 반복 금지**: 이전 댓글에서 "하셧는데"를 사용했다면, 다음 댓글에서는 다른 표현을 사용하세요
   - **⚠️ 절대 반복 금지**: "말씀하신", "하셨는데", "하셧는데" 같은 표현을 연속으로 사용하지 마세요

7. **감정 상태 반영**: ${feelingTone} 상태 유지
...
8. **댓글 길이**: 120-150자 내외 (간결하게, ${currentStyle.tone} 스타일에 맞게)
...
2. **답변 길이 조절**:
   - 모든 댓글: 120-150자 내외 (간결하게, 핵심 정보 포함)
   - ${isLastStep ? '**마지막 댓글**: 자연스럽게 마무리하며 상담 유도 (120-150자 내외)' : ''}
