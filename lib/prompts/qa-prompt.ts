...
export function generateReviewMessagePrompt(
  ...
  // 다양한 후기성 문구 패턴 (매우 다양하게, 항상 친절하고 자세하게)
  const reviewPatterns = [
    // 패턴 1: 가입 완료 + 상세한 감사
    {
      ...
      examples: [
        ...
        '설계사님 덕분에 잘 가입했어요 처음부터 끝까지 친절하게 설명해주셔서 감사합니다',
        '여기서 가입했는데 설명 잘 해주셔서 고맙습니다 궁금한 점마다 하나하나 자세히 답변해주셔서 정말 감사합니다',
        '${productName} 가입했는데 이렇게 친절하게 설명해주셔서 감사합니다 보험료부터 보장 내용까지 자세히 알려주셔서 이해하기 쉬웠어요',
        '설계사님 추천대로 가입했는데 정말 만족스러워요 처음 질문할 때부터 가입까지 전 과정이 만족스러웠습니다',
        '여기서 보험 가입했는데 친절하게 도와주셔서 고맙습니다 설명이 자세해서 보험에 대해 잘 이해할 수 있었어요'
      ]
    },
    // 패턴 2: 만족도 + 상세한 표현
    {
      type: '만족도 + 상세한 표현',
      examples: [
        '설계사님 추천대로 가입했는데 생각보다 좋아서 만족합니다 보험료도 합리적이고 보장도 괜찮아서 정말 좋아요',
        '여기서 가입했는데 보험료도 합리적이고 보장도 괜찮아서 좋아요 설명도 자세해서 이해하기 쉬웠습니다',
        '가입했는데 설명이 자세해서 이해하기 쉬웠어요 감사합니다 처음에는 복잡해 보였는데 하나하나 설명해주셔서 잘 이해할 수 있었습니다',
        '${productName} 가입했는데 보장 내용도 좋고 보험료도 적당해서 만족합니다 설계사님 덕분에 좋은 보험 가입하게 되어서 고맙습니다',
        '설계사님 덕분에 좋은 보험 가입하게 되어서 고맙습니다 처음 질문할 때부터 가입까지 친절하게 도와주셔서 정말 감사합니다',
        '가입했는데 생각보다 괜찮아서 만족스러워요 보험료 대비 보장이 좋아서 만족합니다'
      ]
    },
    // 패턴 3: 추천 의도 + 감사
    {
    ...
    // 패턴 7: 감사 + 앞으로의 관계
    {
      type: '감사 + 앞으로의 관계',
      examples: [
        '여기서 보험 가입했는데 이렇게 가입시켜줘서 정말 고맙습니다 앞으로도 궁금한 점 있으면 문의해도 될까요?',
        '설계사님 덕분에 잘 가입했어요 감사합니다 앞으로도 보험 관련해서 궁금한 점 있으면 편하게 물어봐도 되나요?',
        '가입했는데 설명 잘 해주셔서 고맙습니다 앞으로도 잘 부탁드려요 궁금한 점 있으면 언제든 문의해도 될까요?',
        '${productName} 가입했는데 이렇게 친절하게 설명해주셔서 감사합니다 앞으로도 보험 관련해서 도움이 필요하면 연락해도 되나요?',
        '여기서 가입했는데 친절하게 도와주셔서 고맙습니다 앞으로도 궁금한 점 있으면 편하게 물어봐도 될까요?',
        '가입 완료했고 정말 만족스러워요 앞으로도 보험 관련해서 궁금한 점 있으면 문의해도 되나요?'
      ]
    },
    // 패턴 8: 상세한 후기 + 감사
    {
...
3. **자연스러운 표현**:
   ...
   - **⚠️ 과도하게 짧지 않게, 충분히 자세하게 작성하세요 (100-200자 내외)**

4. **⚠️ 마침표(.) 사용 절대 금지**: 
   - 문장 끝에 마침표를 사용하지 마세요
   - 물음표(?)나 느낌표(!)는 사용 가능하지만, 마침표는 절대 사용하지 마세요

5. **다양성 확보 및 반복 방지 (매우 중요!)**:
   - **⚠️ 절대 반복 금지**: 이전 대화나 다른 후기성 문구와 동일한 내용을 절대 사용하지 마세요
   - **⚠️ 구체적 정보 반복 금지**: "설계사님 덕분에 [나이]세 [성별]에게 딱 맞는 [상품명] 가입할 수 있어서" 같은 구체적 정보를 반복하지 마세요
   - **⚠️ 문구 반복 금지**: "여기서 보험 가입했는데 이렇게 가입시켜줘서 정말 고맙습니다" 같은 문구를 그대로 반복하지 마세요
   - **매번 생성할 때마다 다른 패턴, 다른 표현을 사용하세요**
   - 위의 패턴들을 매번 랜덤하게 선택하여 완전히 다른 문구로 작성하세요
   - 간단하고 자연스러운 감사 인사로 작성하세요 (과도하게 길거나 구체적 정보를 나열하지 마세요)
