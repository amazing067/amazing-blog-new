...
2. **답변 길이 조절**: ${data.answerLength === 'short' ? `
   ...
   - **단계별 답변 길이** (매우 중요!):
     ...
     * 중반 (3-4번째): 구체적으로 답변 (150-250자)
     * 후반 (5번째 이후): 간결하게 답변 (100-200자)
   - 현재 ${stepNumber}번째 댓글이므로 ${stepNumber <= 2 ? '초반 (200-300자)' : stepNumber <= 4 ? '중반 (150-250자)' : '후반 (100-200자)'} 길이로 작성하세요
   - ${isLastStep ? '**마지막 댓글**: 자연스럽게 마무리하며 상담 유도 (후반 길이 기준: 100-200자 내외)' : ''}
   `}
