...
7. **답변 길이 (매우 중요! 첫 답변은 정확히 200-300자 사이로 작성!)**:
   - **첫 답변은 정확히 200-300자 사이로 작성하세요** - 핵심 정보를 포함하면서도 간결하게
   - **⚠️ 매우 중요**: 반드시 200자 이상 300자 이하로 작성하세요
...
export function generateConversationThreadPrompt(
  ...
  const { initialQuestion, firstAnswer, conversationHistory, totalSteps, currentStep, customerRole } = context
  ...
  // 이전 대화 요약
  // 토큰 절감: 대화 히스토리 요약 (RPM 150 제한 대응)
  // 각 메시지를 80자로 제한하여 토큰 사용량 감소
  const previousMessages = conversationHistory
    .map(msg => {
      const role = msg.role === 'customer' ? '고객' : '설계사'
  ...
  if (isCustomerTurn) {
    ...
    const roleConfig = {
      'customer1': {
        role: '질문자',
        description: '원래 질문을 올린 사람입니다. 설계사의 답변을 보고 추가 질문을 하세요.',
        examples: ['암 진단비 최대 1억 5천까지 된다고 하셧는데, 그럼 유사암 진단비는 얼마까지 가능한지 궁금해요', '뇌혈관질환 진단비도 높게 설정 가능한가요?', '2대질환 진단비 말고 다른 특약도 높게 설정 가능한가요'],
        tone: '질문자의 톤: 직접적이고 구체적인 질문, 이전 답변을 참조하며 추가 질문',
        style: '이전 답변에서 언급된 내용을 직접 참조하며 구체적으로 질문하세요. "하셧는데", "말씀하신" 같은 표현 사용'
      },
      'customer2': {
        role: '관심자',
        description: '이 질문에 관심이 있는 다른 사람입니다. 질문자와는 다른 관점에서 댓글을 작성하세요.',
        examples: ['저도 궁금했어요 30대도 어린이보험 가입이 가능한가요?', '이 부분도 궁금해요 유사암 진단비는 얼마까지 되나요?', '감사합니다 도움이 되었어요'],
        tone: '관심자의 톤: "저도", "이 부분도" 같은 표현으로 관심을 표현, 질문자와는 다른 관점',
        style: '질문자와는 다른 관점에서 접근하세요. "저도", "이 부분도", "감사합니다" 같은 표현으로 다른 사람임을 명확히 표현'
      },
      'customer3': {
        role: '비교자',
        description: '다른 보험사와 비교하고 싶어하는 사람입니다. 비교 관점에서 댓글을 작성하세요.',
        examples: ['다른 보험사는 어때요? 비교가 궁금해요', 'A사와 B사 중 어느 게 나을까요?', '성인보험과 비교했을 때 정말 유리한가요?'],
        tone: '비교자의 톤: 비교 관점에서 질문, 다른 상품이나 보험사에 대한 관심',
        style: '비교 관점에서 질문하세요. "다른 보험사", "비교", "어느 게 나을까요" 같은 표현 사용'
      },
      'customer4': {
        role: '확인자',
        description: '설계사의 답변을 확인하고 싶어하는 사람입니다. 확인 관점에서 댓글을 작성하세요.',
        examples: ['정말 그런가요? 확인하고 싶어요', '이 부분도 확인 가능한가요?', '혹시 다른 조건은 없나요?'],
        tone: '확인자의 톤: 확인하고 싶어하는 톤, "정말", "확인", "혹시" 같은 표현',
        style: '확인 관점에서 질문하세요. "정말 그런가요?", "확인하고 싶어요", "혹시" 같은 표현 사용'
      }
    }
    
    const currentRole = customerRole || 'customer1' // 기본값: 질문자
    const roleInfo = roleConfig[currentRole]
    
    // 고객 댓글 생성
    return `당신은 보험 지식이 부족한 일반인이며, 보험카페에서 설계사의 답변을 보고 댓글을 다는 ${roleInfo.role}입니다.

[상황 설정]
- **당신의 역할: ${roleInfo.role}** (매우 중요!)
- **역할 설명: ${roleInfo.description}**
- **나이: ${age}세 (매우 중요!)**
- **성별: ${gender === '남' ? '남성' : '여성'} (매우 중요!)**
- 타겟 고객: ${targetPersona}
- 보험에 대한 지식이 거의 없는 일반인
- 설계사의 답변을 읽고 ${currentRole === 'customer1' ? '추가로 궁금한 점이 생김' : currentRole === 'customer2' ? '관심이 생겨서 댓글을 남김' : currentRole === 'customer3' ? '비교가 궁금해서 댓글을 남김' : '확인하고 싶어서 댓글을 남김'}
...
1. **역할에 맞는 댓글 작성 (매우 중요! 다른 고객이라는 느낌을 명확히!)**:
   - **당신은 ${roleInfo.role}입니다** - ${roleInfo.description}
   - **역할별 톤**: ${roleInfo.tone}
   - **역할별 스타일**: ${roleInfo.style}
   - 역할별 예시:
${roleInfo.examples.map(ex => `     * "${ex}"`).join('\n')}
   - **⚠️ 역할에 맞는 댓글을 작성하세요**:
     * ${currentRole === 'customer1' ? '질문자: 이전 답변을 직접 참조하며 "하셧는데", "말씀하신" 같은 표현 사용, 구체적 추가 질문' : currentRole === 'customer2' ? '관심자: "저도", "이 부분도" 같은 표현으로 다른 사람임을 명확히 표현, 질문자와는 다른 관점' : currentRole === 'customer3' ? '비교자: "다른 보험사", "비교", "어느 게 나을까요" 같은 표현으로 비교 관점 강조' : '확인자: "정말 그런가요?", "확인하고 싶어요", "혹시" 같은 표현으로 확인 관점 강조'}
   - **⚠️ 다른 역할처럼 행동하지 마세요**: ${currentRole === 'customer1' ? '질문자가 아닌 다른 사람처럼 행동하지 마세요. 질문자만 사용하는 표현("하셧는데", "말씀하신")을 사용하세요' : currentRole === 'customer2' ? '관심자가 아닌 다른 사람처럼 행동하지 마세요. "저도", "이 부분도" 같은 표현을 반드시 사용하여 다른 사람임을 명확히 표현하세요' : currentRole === 'customer3' ? '비교자가 아닌 다른 사람처럼 행동하지 마세요. "다른 보험사", "비교" 같은 표현을 반드시 사용하세요' : '확인자가 아닌 다른 사람처럼 행동하지 마세요. "정말", "확인", "혹시" 같은 표현을 반드시 사용하세요'}
   - **⚠️ 매우 중요: 다른 고객이라는 느낌을 명확히 주세요!** 각 역할의 고유한 표현과 톤을 사용하여 질문자와는 다른 사람임을 명확히 표현하세요

2. **고객 스타일 유지 (매우 중요!)**: ${currentStyle.tone} 톤으로 작성하세요
   - 예시:
...
2. **자연스러운 전환 문구 사용 (매우 중요!)**:
   ...
   - **전환 문구 다양화** (매번 다르게 선택, 주요 패턴만):
     * "그렇다면...", "추가로...", "실제로는...", "혹시...", "그런데...", "그리고..."
   - **직접 질문으로 시작** (전환 문구 없이 - 더 자연스러움, 90% 이상):
     * "[담보명] [금액] 적정한가요?", "[담보명]이 [조건]에도 적용되나요?", "[담보명] [금액]보다 더 추가 가입 가능한가요?"
...
7. **댓글 길이 (실제 카페 답변 기준)**: 약 120자 (100-130자 허용, 문장 완성 우선!)
   - **⚠️ 매우 중요**: 문장이 완성되지 않으면 120자 미만이어도 괜찮습니다
   - **절대 문장 중간에 잘리지 않도록 완전한 문장으로 작성하세요**
   - 목표: 120자 정도이지만, 문장 완성이 최우선입니다
...
2. **답변 길이 조절 (실제 카페 답변 기준 - 핵심/요약/정보 모두 포함!)**:
   - **모든 댓글: 약 120자 (100-130자 허용, 문장 완성 우선!)** (초반/중반/후반 모두 동일)
   - ${isLastStep ? '**마지막 댓글**: 자연스럽게 마무리하며 상담 유도 (약 120자, 문장 완성 우선)' : ''}
   - **⚠️ 매우 중요**: 핵심 정보, 요약, 구체적 금액/조건이 모두 포함되어야 합니다
   - **⚠️ 절대 문장 중간에 잘리지 않도록 완전한 문장으로 작성하세요**
   - **⚠️ 문장이 완성되지 않으면 120자 미만이어도 괜찮습니다 - 문장 완성이 최우선입니다!**
   - 목표: 120자 정도이지만, 문장이 자연스럽게 끝나는 지점에서 마무리하세요
...
${searchResultsText}
   ...
   - **${productName}의 구체적 보장 내용 상세 설명**: 고객이 "아 정말 이런 걸 받을 수 있구나"라고 느끼도록 구체적 금액, 조건, 특약명을 명확히
     * 예: "${productName}의 [구체적 특약명]은 [구체적 금액/조건]을 보장해요"
     * 예: "특약 구성이 탄탄한 편입니다 특히 [구체적 특약명]이 있어서 [구체적 보장 내용]을 받을 수 있어요"
   - **경쟁사 비교 분석 (구체적 금액과 예시 필수! 현실적인 금액 사용!)**: 다른 보험사 상품과의 구체적 비교 (보험료, 보장 범위, 특약 구성 등) - 고객이 "아 ${productName}이 더 좋구나"라고 느끼도록
     * **비교 정보를 구체적으로 제시하세요** - 모호한 표현 대신 구체적 숫자와 예시 사용
     * **⚠️ 매우 중요: 현실적인 보험료와 보장 금액을 사용하세요!**
       * 어린이암보험: 월 3-5만원대에 암 진단비 1억원 이상 보장이 일반적
       * 성인암보험: 월 5-7만원대에 암 진단비 1억원 이상 보장이 일반적
       * 상해보험: 월 1-2만원대에 골절 진단비 200-500만원 보장이 일반적
     * 예: "A사는 월 3만원에 암 진단비 1억원 보장, B사는 월 3.5만원에 암 진단비 1억 2천만원 보장인데 ${productName}은 월 ${premiumInfo || '[보험료]'}원으로 암 진단비 1억 5천만원 보장이라 가성비가 좋아요"
     * 예: "성인보험 대비 보험료는 약 20% 저렴합니다 (예: 성인보험 월 5만원 → 어린이보험 월 4만원)"
     * 예: "비슷한 보장 기준으로 비교해보면 삼성생명은 월 3만원대에 암 진단비 1억원, 한화생명은 월 3.5만원대에 암 진단비 1억 2천만원인데 ${productName}은 월 ${premiumInfo || '[보험료]'}원으로 암 진단비 1억 5천만원 보장이라 보장 범위가 넓은 편이에요"
   - **리스크 및 제한사항 명확히 설명**: 보장하지 못하는 부분이나 제한사항을 명확히 설명하되, 부정적이지 않게
     * 예: "다만 이 특약은 면책기간이 90일이어서 참고하시면 좋을 것 같아요"
     * 예: "주의할 점은 비급여 치료비는 별도 특약으로 추가 가입이 가능해요"
...
7. **댓글 형식**:
   ...
   - **이모티콘 사용 (30-50% 확률, 자연스럽게)**:
     * 설계사 댓글: 가끔 이모티콘 사용 (✅, 💡, 📊, 💰, 📋 등) - 전체 댓글의 30-50%만 사용
     * 고객 댓글: 이모티콘 사용 안 함 (자연스러움)
     * 이모티콘은 문단 시작 부분에 배치
     * 과도하게 사용하지 않기 (전체의 30-50%만)
     * 예시: "✅ 질문자님~ 정확히 말씀드리면...", "💡 질문자님~ 뇌혈관 쪽도 당연히..."
