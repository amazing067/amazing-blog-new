...
7. **답변 길이 (매우 중요! 첫 답변은 길고 자세하게 작성 가능!)**:
   - **첫 답변은 길고 자세하게 작성하세요** - 고객의 궁금증을 충분히 해결할 수 있도록 상세하게 설명
   - 질문이 간단하면: 200-300자 (핵심 정보 + 기본 정보 + 상세 설명)
   - 질문이 일반적이면: 300-500자 (핵심 + 기본 + 상세 정보 + 비교)
   - 질문이 복잡하면: 400-700자 (핵심 + 기본 + 상세 + 비교 + 맥락 + 추가 정보)
   - **핵심 정보는 반드시 포함** (구체적 금액, 조건, 특약명 필수)
   - **기본 정보는 가능한 한 포함** (비교, 추가 설명)
   - **상세 정보는 질문 복잡도에 따라 포함** (이유, 맥락)
   - 문단은 3-5개로 구성 (각 문단은 1-2문장)
   - 불필요한 설명이나 반복 표현 제거
   - **예시 구조 (질문 복잡도에 따라 자동 조절)**:
     * 간단한 질문: 호명+공감+핵심 답변 (100-150자) + 구체적 금액/조건 설명 (80-100자) + 간단한 비교 (20-30자) + 상담 유도 (20-30자)
     * 일반적인 질문: 호명+공감+핵심 답변 (100-150자) + 구체적 금액/조건 설명 (100-150자) + 비교 정보 (60-100자) + 추가 맥락 (40-60자) + 상담 유도 (20-30자)
     * 복잡한 질문: 호명+공감+핵심 답변 (150-200자) + 구체적 금액/조건 설명 (150-200자) + 비교 정보 (80-120자) + 추가 맥락 (60-100자) + 상세 설명 (40-60자) + 상담 유도 (20-30자)
...
7. **댓글 길이 (실제 카페 답변 기준)**: 50-100자 내외 (간결하게, ${currentStyle.tone} 스타일에 맞게, 최대 150자)
...
2. **답변 길이 조절 (실제 카페 답변 기준 - 약 120자, 핵심/요약/정보 모두 포함!)**:
   - 초반 (1-2번째): 기본 답변 (100-120자) - 핵심 정보 + 기본 설명
   - 중반 (3-4번째): 간결하게 답변 (80-120자) - 핵심 정보 + 요약
   - 후반 (5번째 이후): 간결하게 답변 (60-120자) - 핵심 정보 중심
   - ${isLastStep ? '**마지막 댓글**: 자연스럽게 마무리하며 상담 유도 (100-150자)' : ''}
   - **⚠️ 매우 중요**: 핵심 정보, 요약, 구체적 금액/조건이 모두 포함되어야 합니다
   - 너무 짧으면 안 되고, 핵심/요약/정보가 다 들어갈 수 있게 작성하세요
