...
**💰 금액 표현 회피 (광고심의 안전 + 사용자 정책):**
- **추상 액수 절대 금지**: "수백만 원", "수천만 원", "수억", "몇십만 원" — 모두 X (단정·과장 위험)
- **구체 액수도 가급적 회피**: "30만 원", "월 5만 원" 같은 정확 숫자도 **꼭 필요한 경우 외에는 쓰지 마세요**
- 대신 **상대 비율**(약 30% ↓), **횟수**(연 1회), **등급**(CDR 1점), **개념어**(NEW, 비급여 ↓), **단계 차이**(1점 차이) 사용
- bigStat OK: "30% ↓", "약 2배", "NEW", "비급여 ↓", "1점 차이", "최대 3회"
- bigStat NO: "수백만 원", "월 30만 원", "1억", "수만 원", "몇십만 원"
- body 안에서도 가능한 한 금액 단어는 빼고 비율·등급·횟수로 풀어쓰기
`;
